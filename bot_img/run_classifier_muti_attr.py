...
import csv
import os
import bot_img.muti_modeling as modeling
...
flags.DEFINE_string(
    "muti_bert_config_file", '../chinese_L-12_H-768_A-12/bert_config.json',
    "The config json file corresponding to the pre-trained BERT model. "
    "This specifies the model architecture.")

flags.DEFINE_string("muti_task_name", 'muti_attr', "The name of the task to train.")

flags.DEFINE_string("muti_vocab_file", '../chinese_L-12_H-768_A-12/vocab.txt',
                    "The vocabulary file that the BERT model was trained on.")

flags.DEFINE_string(
    "muti_output_dir", 'muti_attr_output_2020_1_2/',
    "The output directory where the model checkpoints will be written.")

# Other parameters

flags.DEFINE_string(
    "muti_init_checkpoint", '../chinese_L-12_H-768_A-12/bert_model.ckpt',
    "Initial checkpoint (usually from a pre-trained BERT model).")

flags.DEFINE_bool(
    "muti_do_lower_case", True,
    "Whether to lower case the input text. Should be True for uncased "
    "models and False for cased models.")

flags.DEFINE_integer(
    "muti_max_seq_length", 128,
    "The maximum total input sequence length after WordPiece tokenization. "
    "Sequences longer than this will be truncated, and sequences shorter "
    "than this will be padded.")

flags.DEFINE_bool("muti_do_train", True, "Whether to run training.")

flags.DEFINE_bool("muti_do_eval", False, "Whether to run eval on the dev set.")

flags.DEFINE_bool(
    "muti_do_predict", True,
    "Whether to run the model in inference mode on the test set.")

flags.DEFINE_integer("muti_train_batch_size", 32, "Total batch size for training.")

flags.DEFINE_integer("muti_eval_batch_size", 8, "Total batch size for eval.")

flags.DEFINE_integer("muti_predict_batch_size", 8, "Total batch size for predict.")

flags.DEFINE_float("muti_learning_rate", 5e-5,
                   "The initial learning rate for Adam.")

flags.DEFINE_float("muti_num_train_epochs", 3.0,
                   "Total number of training epochs to perform.")

flags.DEFINE_float(
    "muti_warmup_proportion", 0.1,
    "Proportion of training to perform linear learning rate warmup for. "
    "E.g., 0.1 = 10% of training.")

flags.DEFINE_integer("muti_save_checkpoints_steps", 1000,
                     "How often to save the model checkpoint.")

flags.DEFINE_integer("muti_iterations_per_loop", 1000,
                     "How many steps to make in each estimator call.")

flags.DEFINE_bool("muti_use_tpu", False, "Whether to use TPU or GPU/CPU.")
...
tf.flags.DEFINE_string(
    "muti_gcp_project", None,
    "[Optional] Project name for the Cloud TPU-enabled project. If not "
    "specified, we will attempt to automatically detect the GCE project from "
    "metadata.")

tf.flags.DEFINE_string("muti_master", None, "[Optional] TensorFlow master URL.")

flags.DEFINE_integer(
    "muti_num_tpu_cores", 8,
...
class MutiattrProcessor(DataProcessor):
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    output_weights = tf.get_variable(
        "muti_output_weights", [num_labels, hidden_size],
        initializer=tf.truncated_normal_initializer(stddev=0.02))

    # 如果已存在参数定义相同的变量，就返回已存在的变量，否则创建由参数定义的新变量。
    output_bias = tf.get_variable(
        "muti_output_bias", [num_labels], initializer=tf.zeros_initializer())
...
def main(_):
    ...
    bert_config = modeling.BertConfig.from_json_file(FLAGS.muti_bert_config_file)
    ...
    tokenizer = tokenization.FullTokenizer(
        vocab_file=FLAGS.muti_vocab_file, do_lower_case=FLAGS.muti_do_lower_case)

    tpu_cluster_resolver = None
    if FLAGS.muti_use_tpu and FLAGS.muti_tpu_name:
        tpu_cluster_resolver = tf.contrib.cluster_resolver.TPUClusterResolver(
            FLAGS.muti_tpu_name, zone=FLAGS.muti_tpu_zone, project=FLAGS.muti_gcp_project)
    ...
    if FLAGS.muti_do_train:
        ...
        num_train_steps = int(
            len(train_examples) / FLAGS.muti_train_batch_size * FLAGS.muti_num_train_epochs)
    ...
    if FLAGS.muti_do_train:
        ...
        file_based_convert_examples_to_features(
            train_examples, label_list, FLAGS.muti_max_seq_length, tokenizer, train_file)
    ...
    if FLAGS.muti_do_eval:
        ...
        output_eval_file = os.path.join(FLAGS.muti_output_dir, "eval_results.txt")
    ...
    if FLAGS.muti_do_predict:
        ...
        file_based_convert_examples_to_features(predict_examples, label_list,
                                                FLAGS.muti_max_seq_length, tokenizer,
                                                predict_file)
        ...
        output_predict_file = os.path.join(
            FLAGS.muti_output_dir, "test_results.tsv")
...
if __name__ == "__main__":
    flags.mark_flag_as_required("muti_data_dir")
    flags.mark_flag_as_required("muti_task_name")
    flags.mark_flag_as_required("muti_vocab_file")
    flags.mark_flag_as_required("muti_bert_config_file")
    flags.mark_flag_as_required("muti_output_dir")
    tf.app.run()
